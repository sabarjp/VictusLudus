...
public class ButtonMenu implements ButtonPressListener{
	...
	/*
	 * The node that the tree is expanded up to, and whose direct children we can see
	 */
	...
	public void render(){
		...
		while(e.hasMoreElements()){
			...
			if(!n.isRoot()){
				final GUIImageButton btn = ((ButtonMenuButton) n.getUserObject()).getButton();

				if(n.getLevel() != lastLevel){
					sideCount = 0;
				}

				if(this.growthDirection == EnumDirection.RIGHT){
					btn.setX(this.x + btn.getWidth()*(n.getLevel()-1));
					btn.setY(this.y + btn.getHeight()*sideCount);
				} else if(this.growthDirection == EnumDirection.LEFT){
					btn.setX(this.x + btn.getWidth()*-1*(n.getLevel()-1));
					btn.setY(this.y + btn.getHeight()*sideCount);
				} else if(this.growthDirection == EnumDirection.UP){
					btn.setX(this.x + btn.getWidth()*sideCount);
					btn.setY(this.x + btn.getHeight()*-1*(n.getLevel()-1));
				} else if(this.growthDirection == EnumDirection.DOWN){
					btn.setX(this.x + btn.getWidth()*sideCount);
					btn.setY(this.x + btn.getHeight()*(n.getLevel()-1));
				}

				btn.render();
			}

			lastLevel = n.getLevel();
			sideCount++;
		}
		 */
	}
