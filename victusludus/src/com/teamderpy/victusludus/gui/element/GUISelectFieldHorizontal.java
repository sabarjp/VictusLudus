...
public class GUISelectFieldHorizontal extends GUIElement implements Actionable, Selectable, MouseListener, KeyboardListener, HoverListener, ButtonPressListener, SelectListener{
		...
		this.leftArrowButton.setPressedAction(new Actionable(){
			...
			public void act(){
				if(GUISelectFieldHorizontal.this.currentSelectionNdx > 0){
					...
					VictusLudusGame.engine.eventHandler.signal(new TooltipEvent(this, GUISelectFieldHorizontal.this.getCurrentSelection().getTooltip()));
		...
		if(this.isVisible){
			...
			if (!this.selectionList.isEmpty()){
				...
				if(VictusLudusGame.engine.IS_DEBUGGING){
					...
					VictusLudusGame.engine.graphics.draw(new org.newdawn.slick.geom.Rectangle(this.selectionX, this.selectionY, this.getCurrentSelection().getWidth(), this.getCurrentSelection().getHeight()));
				}
		...
		if(!this.isDisabled() && this.isVisible()){
			if(this.equals(selectEvent.getSource())){
				...
				if(this.selectionList.isEmpty()){
					...
					VictusLudusGame.engine.eventHandler.tooltipHandler.signalAllNow(new TooltipEvent(this, this.getCurrentSelection().getTooltip()));
				}
			}
		}
	}
		...
		if(!this.isDisabled() && this.isVisible()){
			if(this.equals(hoverEvent.getSource())){
				...
				if(this.selectionList.isEmpty()){
					...
					VictusLudusGame.engine.eventHandler.signal(new TooltipEvent(this, this.getCurrentSelection().getTooltip()));
				}
			}
		}
	}
	...
	public void onKeyDown(final KeyDownEvent keyboardEvent) {
		if(this.isSelected){
			if(keyboardEvent.getKey() == Keyboard.KEY_RIGHT){
				...
				VictusLudusGame.engine.eventHandler.signal(new ButtonPressEvent(this, this.getCurrentSelection().getValue()));
			}
		}
	}
	 ...
	 */
	public void setChangedAction(final com.teamderpy.VictusLudusGame.enginengine.Actionable act){
		this.action = act;
	}
