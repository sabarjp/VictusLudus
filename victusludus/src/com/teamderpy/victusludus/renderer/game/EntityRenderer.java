...
public class EntityRenderer implements RenderListener {
	...
	public void calculateCulledEntity (final GameEntity gameEntity, final int layer) {
		...
		int[] sc = RenderUtil.worldCoordToRawUnscaledScreenCoord(this.gameRenderer.game, gameEntity.getPos().getX(), gameEntity
			.getPos().getY(), gameEntity.getPos().getZ());
		// System.err.println("base of entity is " + gameEntity.getPos() + "  " +
// sc[0] + "," + sc[1]);
		// int[] rrr = RenderUtil.screenCoordToRawUnscaledWorldCoord(sc[0], sc[1],
// gameEntity.getPos().getZ());
		// System.err.println("   confirm at " + sc[0] + "," + sc[1] +
// " for the layer " + gameEntity.getPos().getZ() + " the potential tile is " +
// rrr[0] + "," + rrr[1] + "," + rrr[2]);
		...
		for (int i = yOffsets.length - 1; i >= 0; i--) {
			// System.err.println("  checking against offset " + i + " at " +
// yOffsets[i]);
			isBlocked = false;

			// System.err.println("  making sure that offset " + (i+1+z) +
// " isnt over current layer " + layer);
			if (i + 1 + z > layer) {
				// this entity must be visible since nothing could break sight of
// its top
			...
			for (int k = layer; k >= z + i + 1; k--) {
				...
				// System.err.println("    at " + sc[0] + "," + yOffsets[i] +
// " for the layer " + k + " the potential tile is " + wc[0] + "," + wc[1] + ","
// + wc[2]);
				...
				if (wc[0] >= 0 && wc[1] >= 0 && wc[0] < tileArray[k].length && wc[1] < tileArray[k][0].length) {
					...
					if (t != null) {
						// System.err.println("       a blocking tile exists for layer "
// + k);
		...
		// if we got all the way through then the entity is fully blocked off, so
// remove it
		if (isBlocked) {
	 ...
	 */
	@Deprecated
	public void render (final SpriteBatch batch, final float deltaT, final Vector<GameEntity> entityList, final int layer) {
		...
		for (GameEntity gameEntity : this.culledEntityList) {
			ScreenCoord c = RenderUtil.worldCoordToScreenCoord(this.gameRenderer.game, gameEntity.getPos().getX(), gameEntity
				.getPos().getY(), gameEntity.getPos().getZ());

			if (c.x < 0 || c.y < 0 || c.x > this.gameRenderer.game.getGameDimensions().getWidth()
				|| c.y > this.gameRenderer.game.getGameDimensions().getHeight()) {
				continue; // do not render entities off the screen!
			}

			TextureRegion a = gameEntity.getCurrentAnimation(deltaT);
			if (a != null) {
				c.y -= a.getRegionHeight() * this.gameRenderer.game.getGameCamera().getZoom();
				c.y += this.gameRenderer.game.getTileHeightScaled();

				batch.draw(a, c.x, c.y, a.getRegionWidth() * this.gameRenderer.game.getGameCamera().getZoom(), a.getRegionHeight()
					* this.gameRenderer.game.getGameCamera().getZoom());
			}
