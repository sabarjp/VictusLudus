...
public final class VFile {
	...
	/**
	 * Returns a Gdx FileHandle appropriate for the
	 * platform
	 * 
	...
	public static FileHandle getFileHandle(String path){
	   ...
	   case Desktop:
	      // desktop specific code
	   	if(Gdx.files.local(ASSETS_PATH + path).exists()){
	   		return Gdx.files.local(ASSETS_PATH + path);
	   	} else {
	...
	/**
	 * Gets the children of a directory, trying to use list
	 ...
	 */
	private static FileHandle[] getChildrenOnCondition(FileHandle directory, boolean getFiles, boolean getDirectories){
		FileHandle listFile;
		...
		if(directory.type() == FileType.Classpath){
			...
			if(listFile.exists()){
				return createFileListFromListFile(listFile, getFiles, getDirectories);
			}
	 ...
	 * @return an array full of file handles of the children of the directory
	 */
	public static FileHandle[] getChildren(FileHandle directory){
		return getChildrenOnCondition(directory, true, true);
	}
	
	/**
	 * Gets only files of a directory, trying to use list
	 * files where possible.
	 * 
	 * @param directory a file handle to the directory
	 * @return an array full of file handles of the children of the directory
	 */
	public static FileHandle[] getFiles(FileHandle directory){
		return getChildrenOnCondition(directory, true, false);
	}
	
	/**
	 * Gets the sub-dirs of a directory, trying to use list
	 * files where possible.
	 * 
	 * @param directory a file handle to the directory
	 * @return an array full of file handles of the children of the directory
	 */
	public static FileHandle[] getSubDirectories(FileHandle directory){
		return getChildrenOnCondition(directory, false, true);
	}
	
	/**
	 * Creates an array list of file handles that a list
	 * file points to.  A list file lists all the files and
	 * folders in the directory that it resides in.
	 * 
	 ...
	 */
	private static FileHandle[] createFileListFromListFile(FileHandle listFile, boolean getFiles, boolean getDirectories){
		Array<FileHandle> files = new Array<FileHandle>(new FileHandle[0]);
