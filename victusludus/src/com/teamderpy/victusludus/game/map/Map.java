...
import com.teamderpy.victusludus.math.heightmap.LayeredGenerator;
...
public class Map {
	...
	public Map (final GameSettings requestedSettings, final Game game) {
		...
		LayeredGenerator gen = new LayeredGenerator((int)requestedSettings.getRequestedMapScale(),
			requestedSettings.getRequestedMapRandomness());
		int[][] mapArray = gen.generateInt(this.width, this.height, true);
		MatrixMath.smooth(mapArray, (int)requestedSettings.getRequestedMapSmoothness());
		...
		MatrixMath.capMax(mapArray,
			(int)(lowestPoint + (highestPoint - lowestPoint) * (1.0F - requestedSettings.getRequestedMapPlateauFactor())));
		MatrixMath.capMin(mapArray,
			(int)(lowestPoint + (highestPoint - lowestPoint) * requestedSettings.getRequestedMapPlateauFactor()));
