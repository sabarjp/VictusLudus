...
public class GUITextField extends GUIElement implements Actionable, Selectable, MouseListener, KeyboardListener, HoverListener, SelectListener, FocusListener{
	...
	public GUITextField(final int x, final int y, final int width, final Color color, final UnicodeFont font) {
		super(x, y, (int)(width * VictusLudus.e.scalingFactor) + GUITextField.BORDER_SIZE + GUITextField.BORDER_SIZE, font.getLineHeight() + GUITextField.BORDER_SIZE + GUITextField.BORDER_SIZE);
		...
		if(this.isVisible()){
			...
			if(this.isDisabled){
				...
				if(this.isBlinkVisible){
					...
					VictusLudus.e.graphics.setColor(this.color);
					VictusLudus.e.graphics.fill(new Rectangle(this.x+xblink+GUITextField.BORDER_SIZE,this.y+GUITextField.BORDER_SIZE,2,this.getHeight() - GUITextField.BORDER_SIZE - GUITextField.BORDER_SIZE));
	...
	public void onKeyPress(final KeyboardEvent keyboardEvent) {
		if(keyboardEvent.getKey() == Keyboard.KEY_RIGHT){
			...
			if(this.isFocused){
				VictusLudus.e.eventHandler.focusHandler.signalAllNow(new FocusEvent(this, false));
			}
		}
			...
			if(this.isFocused){
				...
				VictusLudus.e.eventHandler.focusHandler.signalAllNow(new FocusEvent(this, true));
		...
		if(mouseEvent.getButton() == MouseEvent.BUTTON_1){
			if(mouseEvent.isButtonPressed()){
				if(this.isInside(mouseEvent.getX(),mouseEvent.getY())){
					if(!this.isFocused){
						VictusLudus.e.eventHandler.signal(new FocusEvent(this, true));
					}
					...
					if(this.isFocused){
						VictusLudus.e.eventHandler.signal(new FocusEvent(this, false));
					}
				}
			}
		}
	...
	public void registerListeners() {
		...
		VictusLudus.e.eventHandler.focusHandler.registerPlease(this);
	...
	public void unregisterListeners(){
		...
		VictusLudus.e.eventHandler.focusHandler.unregisterPlease(this);
	 ...
	 */
	public void setTypingAction(final com.teamderpy.victusludus.engine.Actionable act){
		this.action = act;
	}
